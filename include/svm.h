...
#define SVM_EXIT_MSR      0x7C /* RDMSR/WRMSR intercepted */
#define SVM_EXIT_VMMCALL  0x81
#define SVM_EXIT_RDTSCP   0x87
#define SVM_EXIT_SHUTDOWN 0x7F
#define SVM_EXIT_INTR     0x60 /* Physical INTR intercepted */
#define SVM_EXIT_INIT     0x63 /* INIT request intercepted */
#define SVM_EXIT_RDPMC    0x6F
#define SVM_EXIT_NPF      0x400 /* Nested Page Fault */
#define SVM_EXIT_INVALID  0xFFFFFFFFFFFFFFFFULL

/* event_inject field (VMCB control offset 0x0A8) */
#define SVM_EVENTINJ_VALID       (1ULL << 31)
#define SVM_EVENTINJ_TYPE_EXTINT (0ULL << 8) /* external interrupt */
...
/* intercept_misc1 */
#define SVM_ICEPT_INTR (1U << 0) /* physical INTR */
#define SVM_ICEPT_INIT (1U << 3) /* INIT request */
...
struct __packed SvmControl {
    ...
    uint8_t  _pad2[0x10];      /* 0x0B8 – 0x0C7 */
    uint64_t n_rip;            /* 0x0C8  next sequential RIP (requires NRIP_SAVE) */
    uint8_t  _pad3[0x330];     /* 0x0D0 – 0x3FF */
}; /* total: 0x400 bytes */
