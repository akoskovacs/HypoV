...
int vmx_read_capabilities(struct VmxCapabilities *caps)
{
    ...
    caps->vc_vmcs_size   = (uint16_t)((basic >> VMX_BASIC_VMCS_SIZE_SHIFT)
    ...
    if (caps->vc_proc_ctls & PROC_ACTIVATE_SECONDARY) {
        ...
        uint32_t wanted2 = 0;
        if (caps->vc_ept)                wanted2 |= PROC2_EPT;
        if (caps->vc_vpid)               wanted2 |= PROC2_VPID;
        if (caps->vc_unrestricted_guest) wanted2 |= PROC2_UNRESTRICTED_GUEST;
        if (proc2_may1 & PROC2_RDTSCP)  wanted2 |= PROC2_RDTSCP;
