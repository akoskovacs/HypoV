...
XZ_EXTERN struct xz_dec_lzma2 *xz_dec_lzma2_create(enum xz_mode mode,
						   uint32_t dict_max)
{
	struct xz_dec_lzma2 *s = mm_expand_heap(sizeof(*s));
	if (s == NULL)
		return NULL;
	...
	if (DEC_IS_PREALLOC(mode)) {
		s->dict.buf = mm_expand_heap(dict_max);
		if (s->dict.buf == NULL) {
			kfree(s);
			return NULL;
		}
	...
	if (DEC_IS_MULTI(s->dict.mode)) {
		...
		if (DEC_IS_DYNALLOC(s->dict.mode)) {
			if (s->dict.allocated < s->dict.size) {
				// NOTICE: No free is needed now
				// vfree(s->dict.buf);
				s->dict.buf = mm_expand_heap(s->dict.size);
				if (s->dict.buf == NULL) {
					s->dict.allocated = 0;
					return XZ_MEM_ERROR;
				}
